...
### Provide:
- A short README.md explaining usage (installation, function examples, CLI usage).
- Inline docstrings describing each function, its parameters, and output schema.

### Publishing to PyPI

You are required to publish the datagen library to PyPI so it can be installed via pip.


### Deliverables
1). A complete datagen Python package ( Give it a name of your choice hosted on GitHub).
2). A PyPI link to the published package.
3). Example notebooks or scripts demonstrating how to use each generator.
4). A README.md file with installation and usage instructions.
